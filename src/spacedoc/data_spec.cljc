...
(defmulti ^:private  node :tag)
(defmethod node :bold [_] ::bold)
(defmethod node :center [_] ::center)
(defmethod node :kbd [_] ::kbd)
(defmethod node :example [_] ::example)
(defmethod node :fixed-width [_] ::fixed-width)
(defmethod node :headline [_] ::headline)
(defmethod node :headline-level-1 [_] ::headline-level-1)
(defmethod node :headline-level-2 [_] ::headline-level-2)
(defmethod node :headline-level-3 [_] ::headline-level-3)
(defmethod node :headline-level-4 [_] ::headline-level-4)
(defmethod node :headline-level-5 [_] ::headline-level-5)
(defmethod node :headline-level-6 [_] ::headline-level-6)
(defmethod node :todo [_] ::todo)
(defmethod node :description [_] ::description)
...
(defmethod node :embeddable-file-path [_] ::embeddable-file-path)
(defmethod node :file-path [_] ::file-path)
(defmethod node :embeddable-web-link [_] ::embeddable-web-link)
(defmethod node :web-link [_] ::web-link)
(defmethod node :paragraph [_] ::paragraph)
(defmethod node :feature-list [_] ::feature-list)
(defmethod node :plain-list [_] ::plain-list)
(defmethod node :plain-text [_] ::plain-text)
(defmethod node :quote [_] ::quote)
(defmethod node :section [_] ::section)
(defmethod node :src [_] ::src)
(defmethod node :strike-through [_] ::strike-through)
(defmethod node :subscript [_] ::subscript)
(defmethod node :superscript [_] ::superscript)
(defmethod node :table [_] ::table)
(defmethod node :table-cell [_] ::table-cell)
(defmethod node :table-row [_] ::table-row)
(defmethod node :root [_] ::root)
(defmethod node :underline [_] ::underline)
(defmethod node :verbatim [_] ::verbatim)
(defmethod node :verse [_] ::verse)
(defmethod node :default [_] nil)
(s/def ::node (s/multi-spec node :tag))
