(ns spacetools.spacedoc.data.node-test
  (:require #_ [clojure.spec.gen.alpha :as gen]
            ...
            [spacetools.spacedoc.data.node :refer :all]
            [spacetools.spacedoc.shared :refer [samples make-f-spec-reper]]))


(doall
 (for [v (vals (ns-publics 'spacetools.spacedoc.data.node))
         ...
         (testing (str "Node constructor function \"" ~v "\" speced.")
           (is (s/spec? f-spec#)
               (format (str "Public function `%s` doesn't have spec\n"
                            "All public functions in the `spacetools.spacedoc.data.node`"
