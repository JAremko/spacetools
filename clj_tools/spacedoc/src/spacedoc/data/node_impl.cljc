...
(defmacro defnode
  ...
  Do not create constructor ff CON? argument is provided with `false` value.
  ...
  - When generating constructors the children specs can only be
    `s/coll-of` or `s/cat` forms."
  ([k spec-form] (defnode &form &env k false spec-form))
  ([k con? spec-form]
   (let [tag (u/unqualify k)
         ...
         arg-l (when-not con? (defnode-spec-args q-ks))]
     `(do
        ;; Register node
